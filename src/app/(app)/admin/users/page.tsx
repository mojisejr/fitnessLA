...
function mapManagedUserToDeleteResult(user: ManagedStaffUserRecord): DeleteManagedUserResult {
    if (user.role !== "ADMIN" && user.role !== "CASHIER" && user.role !== "TRAINER") {
        throw new Error("ลบได้เฉพาะ admin, cashier และ trainer เท่านั้น");
    }
...
export default function AdminUsersPage() {
    ...
    const [role, setRole] = useState<"OWNER" | "ADMIN" | "CASHIER" | "TRAINER">("CASHIER");
    ...
    const loadOwnerData = useCallback(async () => {
        ...
        try {
            ...
            setLoadError(getErrorMessage(error, "ไม่สามารถโหลดรายชื่อผู้ใช้ได้"));
        } finally {
            setIsLoadingData(false);
        }
    }, [adapter.mode]);

    const loadAttendanceDeviceStatus = useCallback(async () => {
        if (adapter.mode !== "real") {
    ...
    }, [loadAttendanceDeviceStatus, loadOwnerData]);
            ...
            <div className="space-y-6">
                <section className="rounded-[28px] border border-line bg-surface-strong p-6 md:p-8">
                    ...
                    <p className="mt-3 max-w-4xl text-sm leading-7 text-muted">
                        owner ใช้หน้านี้สร้าง user สำหรับ admin, cashier และ trainer, กำหนดเวลาเข้างานและออกงานให้ทีมหน้าร้าน, อนุมัติเครื่องสำหรับลงชื่อเข้างาน และตรวจตาราง attendance ย้อนหลังจากฐานข้อมูลจริง โดยการอนุมัติเครื่องนี้ใช้บังคับเฉพาะตอนกดเข้างาน ไม่ได้บล็อกการ login จากเครื่องอื่น
                    </p>
                </section>
                ...
                <section className="grid gap-4 md:grid-cols-3">
                    <div className="rounded-[28px] border border-line bg-surface-strong p-6">
                        ...
                        <p className="mt-2 text-sm text-muted">รวม admin, cashier และ trainer ที่ owner จัดการได้จากฐานจริง</p>
                    </div>
                    <div className="rounded-[28px] border border-line bg-surface-strong p-6">
                    ...
                    <section className="rounded-[28px] border border-line bg-surface-strong p-6 md:p-8">
                        ...
                        <form className="mt-5 space-y-4" onSubmit={handleCreateUser}>
                            ...
                            <div className="grid gap-4 xl:grid-cols-2">
                                ...
                                <select
                                    aria-label="บทบาท"
                                    value={role}
                                    onChange={(event) => setRole(event.target.value as "OWNER" | "ADMIN" | "CASHIER" | "TRAINER")}
                            ...
                            <p className="text-sm leading-7 text-muted">
                                ถ้าสร้าง role เทรนเนอร์ บัญชีนี้จะ login ได้ทันที และ owner ต้องไปผูกในหน้าเทรนเนอร์อีกขั้นเพื่อให้เห็นข้อมูลของตัวเองเท่านั้น ส่วนทีมหน้าร้านจะใช้เครื่องลงเวลาร่วมกับเครื่องที่ owner อนุมัติไว้ด้านซ้าย
                            </p>
                            <button
                                type="submit"
                        ...
                        <div className="mt-6 rounded-3xl border border-line bg-background/70 p-5">
                            ...
                            <ul className="mt-3 space-y-2 text-sm leading-7 text-foreground">
                                ...
                                <li>trainer login ได้ แต่จะเปิดใช้งานได้เฉพาะหน้าเทรนเนอร์ของตัวเองหลัง owner ผูกบัญชีเข้ากับโปรไฟล์เทรนเนอร์แล้ว</li>
                ...
                <section className="rounded-[28px] border border-line bg-surface-strong p-6 md:p-8">
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            <p className="text-xs uppercase tracking-[0.16em] text-muted">รายชื่อผู้ใช้ที่ owner จัดการ</p>
                            <h2 className="mt-2 text-2xl font-semibold text-foreground">กำหนดเวลางานและดูสถานะผู้ใช้</h2>
                        </div>
                    ...
                    {isLoadingData ? (
                        ...
                        <div className="mt-6 rounded-3xl border border-dashed border-line bg-background p-6 text-sm leading-7 text-muted">
                            ยังไม่มี user ที่ owner จัดการในฐานข้อมูลจริง
                        </div>
                    ) : (
                                ...
                                return (
                                    <div key={userId} className="rounded-3xl border border-line bg-background/70 p-5">
                                        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="mt-1 text-sm text-muted">เวลางาน: {user.role === "TRAINER" ? "ไม่ได้ใช้ attendance ทีมหน้าร้าน" : formatScheduleWindow(user)}</p>
                                        ...
                                        <p className="mt-3 text-xs leading-6 text-muted">
                                            {user.role === "TRAINER"
                                                ? "บัญชี trainer จะไม่ถูกนับใน attendance ทีมหน้าร้าน และต้องไปผูกในหน้าเทรนเนอร์เพื่อเปิดสิทธิ์ดูข้อมูลของตัวเอง"
