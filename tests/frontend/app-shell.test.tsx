...
describe("AppShell", () => {
    ...
    it("hides shift status details for trainer users", async () => {
        mockUsePathname.mockReturnValue("/trainers");
        mockUseAuth.mockReturnValue({
            session: {
                user_id: "u4",
                username: "oooo",
                full_name: "Trainer Smoke",
                role: "TRAINER",
        ...
        expect(screen.getByText("Trainer Smoke")).toBeInTheDocument();
        expect(screen.queryByText("สถานะกะ")).not.toBeInTheDocument();
        expect(screen.queryByText("มีกะที่เปิดอยู่")).not.toBeInTheDocument();
        expect(screen.queryByText("เงินที่ทำได้")).not.toBeInTheDocument();
        expect(mockAdapter.getDailySummary).not.toHaveBeenCalled();
        expect(screen.getByRole("link", { name: /เทรนเนอร์เปิด/i })).toBeInTheDocument();
    });
});
