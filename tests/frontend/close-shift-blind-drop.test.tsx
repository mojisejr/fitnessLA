...
describe("close shift blind drop", () => {
    ...
    it("renders close shift flow when active shift exists even if session shift id is stale", async () => {
        clearMockSession();
        seedMockSession({
            session: {
                user_id: 3,
        ...
        expect(screen.getByRole("heading", { name: "ปิดกะ" })).toBeInTheDocument();
        expect(screen.queryByText("หน้านี้เริ่มใช้งานได้เมื่อมีกะที่เปิดอยู่")).not.toBeInTheDocument();
    }, 30000);
});